...
# Map Wuhan epitopes to the above regions of Omicron
...
- For each epitope of interest, scan the mutant proteome to see if we find epitope match or a mutant.
-
```{r}
...
# Nested foreach: for each epitope, run through each variant protein sequence.
...
stopCluster(cl)
# Take the highest match score for each peptide. This is their match in the variant proteome.
...
```{r}
# Get rid of epitopes with Hamming = 0, i.e, identical WT-MT
...
# Read in known SNPs to validate our findings. NAs are retained as they remain observed mutants.
...
# Summarise the peptide list, so group same positions of orf1a/orf1ab
...
- For WT and MT, read in netMHCPAN predictions against 64 HLA-I alleles
## WT wuhan peptides
