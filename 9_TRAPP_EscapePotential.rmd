...
data_path <- TEST_DATA_LOCATION # set working directory
files <- dir(data_path, pattern = "NetMHC_Results.csv") # get all files in the directory

data3 <- data_frame(file = files) %>% # create a data frame with the file names and file contents
  mutate(file_contents = map(file,
                             ~ fread(file.path(data_path, .),skip = 1))
  )

Netmhcpanres <- unnest(data3)  # unnest the file contents
Netmhcpanres=Netmhcpanres %>% mutate(HLA_Allele = gsub(x=Netmhcpanres$file,pattern="Allele_|_NetMHC_Results.csv",replacement = "")) # Extract HLA Allele
Netmhcpanres=Netmhcpanres %>% select(! c(file,Pos,ID,core,icore)) %>% mutate(Binder = ifelse(NB==1,"BINDER","NONBINDER")) # Remove unecessary columns and create a binder binary column
Netmhcpanres=Netmhcpanres %>% mutate(HLA_Allele = gsub(x=HLA_Allele,pattern = "\\:",replacement = "")) # Remove : from HLA Allele

NETMHC_CLASSI=Netmhcpanres # Rename for clarity
NETMHC_CLASSI=NETMHC_CLASSI %>% mutate(MT_nM_41 = round(50000^(1-`BA-score`) ,digits=5)) # Calculate MT binding affinity in nM
NETMHC_CLASSI=NETMHC_CLASSI %>% mutate(Binder = ifelse(BA_Rank>=2, "NONBINDER","BINDER")) # Create a binary column for binder/nonbinder based on BA rank

MUTAGENESIS_BINDERS=NETMHC_CLASSI %>% select(Peptide, HLA_Allele, BA_Rank,Binder)%>% mutate(Dataset="Mutagenesis")%>% distinct() # Get all required columns
...
WUHAN_PEPTIDES_BINDING_SCORES=readRDS("ORIGINAL_PEPTIDES_BINDING_SCORES_V2.rds") # read in original wuhan pMHC
WUHAN_PEPTIDES_BINDING_SCORES=WUHAN_PEPTIDES_BINDING_SCORES %>% filter(Peptide %in% MUTATIONS$Peptide) # Filter to only include pMHC with mutations in BA1
...
WUHAN_PEPTIDES_BINDING_SCORES=WUHAN_PEPTIDES_BINDING_SCORES %>%
        select(Peptide, Predicted_Binding, BA_Rank, Binder)%>% # Select required columns
        separate_rows_(cols = c("Predicted_Binding","BA_Rank","Binder"),sep=",")%>% mutate(Dataset = "WuhanTest") # Separate rows based on comma, and add a dataset column - labelled as WuhanTest

WUHAN_PEPTIDES_BINDING_SCORES=WUHAN_PEPTIDES_BINDING_SCORES %>% dplyr::rename(AASeq1=Peptide)%>% select(!Dataset) # Rename Peptide column to AASeq1, and remove dataset column (as AASeq1 is the wt peptide)
```
...
OMI_PEPTIDES_BINDING_SCORES=readRDS("OMICRON_PEPTIDES_BINDING_SCORES_V2.rds") # Read in Omicron pMHC scores
...
# So find the correct scores and produce the data table
BINDER_BINDER=ANTIJOIN_SET %>% mutate(GROUP = paste0(Binder, "_",MT_Binder)) %>% filter(GROUP=="BINDER_BINDER")

BINDER_BINDER=BINDER_BINDER %>%
        left_join(fread(cmd = "unzip -p TRAPP/DATA_V3_SARS2_ONLY/pred_scores/prott5_xl_bfd_peptides_sars2_mut_test_prediction_withlabel_2.txt.zip")%>%
                          distinct()%>%
...
FULL_PREDICTIONS_DT %>% dplyr::rename("Variant Peptide"=AASeq2, "HLA Allele"=Predicted_Binding,MutantScore=Prediction,TRAPP_Wuhan=TRAPP_Prediction)%>%
    readr::write_csv(file="/Users/paulbuckley/Nexus365/WIMM CCB - Koohy Group - Documents/Koohy Group/Effect_Mutation_Tcells/V11/SUBMITTED/R_R/Supplementary Datafile/Table5_MutagenesisPredictions.csv")
...
```{r}
# function to generate the neighbor network connecred sub graph
neighborNetwork_ConnectedSubGraph <- function(neighborNetResult, peptide){ # peptide is the peptide of interest
  connectedPeptides <- c( # find all the connected peptides
    peptide,
  ...
  connectedSubgraph <- igraph::induced_subgraph( # produce the connected subgraph
...
MUTS_DT%>% select(Peptide, logsOdds, se)%>% dplyr::rename(EscapeScore=logsOdds) %>%
        readr::write_csv(file="/Users/paulbuckley/Nexus365/WIMM CCB - Koohy Group - Documents/Koohy Group/Effect_Mutation_Tcells/V11/SUBMITTED/R_R/Supplementary Datafile/Table6_EscapeScores.csv")
...
# Does fraction hydrophobicity associate with transition potential?
-  in TCR contact vs. anchor


```{r}

HYDROPHOBIC_RESIDUES=chemistry %>% filter(group == "Hydrophobic")%>% pull(letter) # find hydrophobic residues

NINEMERS=MUTS_DT %>% filter(nchar(Peptide)==9) %>% mutate(TCRContact_Peptide = substr(Peptide, start=3, stop=8)) # get 9mers tcr contact positions
TENMERS = MUTS_DT %>% filter(nchar(Peptide)==10)%>% mutate(TCRContact_Peptide = substr(Peptide, start=3, stop=9)) # get 10mers tcr contact positions

MUTS_DT=NINEMERS %>% rbind(TENMERS) # combine to generate MUTS DT
...
MUTS_DT=MUTS_DT %>% mutate(TCR_HydrophobicCount =  stringi::stri_count_regex(MUTS_DT$TCRContact_Peptide, paste0(HYDROPHOBIC_RESIDUES,collapse = "|"))) %>%
        mutate(TCR_HydroFraction = TCR_HydrophobicCount/nchar(TCRContact_Peptide))%>% select(!TCR_HydrophobicCount) # calculate hydrophobicity fraction in TCr contact positions

MUTS_DT=MUTS_DT %>% mutate(NonContactRes = stringr::str_remove(Peptide, TCRContact_Peptide)) # get residues not in contact with TCR
MUTS_DT=MUTS_DT %>% mutate(NC_HydrophobicCount =  stringi::stri_count_regex(MUTS_DT$NonContactRes, paste0(HYDROPHOBIC_RESIDUES,collapse = "|"))) %>%
        mutate(NC_HydroFraction = NC_HydrophobicCount/nchar(NonContactRes))%>% select(!NC_HydrophobicCount) # calculate hydrophobicity fraction in non contact positions
...
HIGH_SET = MUTS_DT%>% slice_max(order_by = logsOdds, n=9,with_ties = FALSE) %>% mutate(Dataset = "High")
LOW_SET = MUTS_DT%>% slice_min(order_by = logsOdds, n=9, with_ties = FALSE) %>% mutate(Dataset = "Negative")

LOW_HIGH_DT= LOW_SET %>% rbind(HIGH_SET) %>% distinct()
LOW_HIGH_DT %>% DT::datatable()
...
REMAINING_CHEMISTRY_MUTS_DT=foreach::foreach(i=1:length(unique(REMAINING_CHEMISTRY$group)), .combine = "rbind", .packages = c("dplyr","data.table"))%do% {
  CHEMISTRY_GRP = unique(REMAINING_CHEMISTRY$group)[i] # get group of chemistry i.e polar, etc
  CHEMISTRY_AAS = REMAINING_CHEMISTRY %>% filter(group == CHEMISTRY_GRP)%>% pull(letter)
  MUTS_DT %>% select(Peptide,logsOdds, TCRContact_Peptide, NonContactRes)%>%
          mutate(NC_Count =  stringi::stri_count_regex(MUTS_DT$NonContactRes, paste0(CHEMISTRY_AAS,collapse = "|"))) %>% # calculate number of residues in group in non contact positions
          mutate(NC_Fraction = NC_Count/nchar(NonContactRes))%>% select(!Anchor_Count)%>% mutate(group=CHEMISTRY_GRP)%>% # calculate fraction of residues in chemistry group in non contact positions
          mutate(TCRContact_Count =  stringi::stri_count_regex(MUTS_DT$TCRContact_Peptide, paste0(CHEMISTRY_AAS,collapse = "|"))) %>% # calculate number of residues in group in TCR contact positions
          mutate(TCRContact_Fraction = TCRContact_Count/nchar(TCRContact_Peptide))%>% select(!TCRContact_Count)%>% mutate(group=CHEMISTRY_GRP) # calculate fraction of residues in chemistry group in TCR contact positions
}

LOW_SET = REMAINING_CHEMISTRY_MUTS_DT%>% filter(Peptide %in% LOW_SET$Peptide)%>% mutate(Dataset = "Negative") # label low set i.e with low escape potential
HIGH_SET = REMAINING_CHEMISTRY_MUTS_DT%>% filter(Peptide %in% HIGH_SET$Peptide)%>% mutate(Dataset = "High") # label high set i.e with high escape potential
TOLERANT_SET = REMAINING_CHEMISTRY_MUTS_DT%>% filter(Peptide %in% c("TLACFVLAAV","YYHKNNKSW","FGEVFNATRF","QTGKIADYNY","QWNLVIGFLF","GEVFNATRF","FQPTNGVGY",
                                                                    "SYQTQTNSPR","DSKVGGNYNY"))%>% mutate(Dataset = "Negligible") # select tolerant set of peptides i.e those with escpe potential and se around zero
# Bind together DT of proportiond of selected epitopes with high, neg, low escpe potential and fraction of chemistry group residues in TCr vs non cointact positions
LOW_HIGH_DT= LOW_SET %>% rbind(HIGH_SET)%>% rbind(TOLERANT_SET)%>% mutate(Dataset = factor(Dataset, levels = c("High","Negligible","Negative")))
HYDROPHOBICITY_ANCHORFREQ_PLT=LOW_HIGH_DT %>% filter(group == "Hydrophobic") %>% distinct()%>%
        mutate(Anchor_Fraction = round(Anchor_Fraction,digits=3))%>%
...
# Calculate real (observational) data
NINE_LOW_HIGH_DT = LOW_HIGH_DT%>% select(Peptide, Dataset)%>% distinct() %>% filter(nchar(Peptide)==9) # select peptides with 9 residues
NINE_LOW_HIGH_DT=NINE_LOW_HIGH_DT%>% cbind(stringr::str_split_fixed(NINE_LOW_HIGH_DT$Peptide,n=9,pattern = "")) # split into columns
TEN_LOW_HIGH_DT = LOW_HIGH_DT%>% select(Peptide, Dataset)%>% distinct() %>% filter(nchar(Peptide)==10) # select peptides with 10 residues
TEN_LOW_HIGH_DT=TEN_LOW_HIGH_DT%>% cbind(stringr::str_split_fixed(TEN_LOW_HIGH_DT$Peptide,n=10,pattern = "")) # split into columns


```

```{r,dpi=300}
# Generate a background distribution of frequency of amino acids in the 9mer and 10mer peptides, as above for observed
...
BACKGROUND_AA_DISTR=NINEMER_AA_BACKGROUND %>% mutate("10"="-") %>% rbind(TENMER_AA_BACKGROUND) # combine background into one data frame
BACKGROUND_AA_DISTR=BACKGROUND_AA_DISTR%>%
        pivot_longer(cols = `1`:`10`)%>%dplyr::rename(letter=value,position=name)%>% ungroup() # convert to long format

SAMPLED_BACKGROUND_AA_DISTR=foreach::foreach(i=1:10, .combine = "rbind", .packages = c("dplyr","data.table"))%do% {
    SAMPLED_PEPS=BACKGROUND_AA_DISTR %>% select(Peptide) %>% distinct()%>% sample_n(size=5)%>% pull(Peptide) # shuffle dataset to generate stat background
    BACKGROUND_AA_DISTR %>% filter(Peptide %in% SAMPLED_PEPS)%>% group_by(letter)%>% dplyr::summarise(n=n())%>% mutate(ID=i) # calculate frequency of each amino acid
}

SAMPLED_BACKGROUND_AA_DISTR=SAMPLED_BACKGROUND_AA_DISTR %>% mutate(Dataset = "CTRL")%>% select(!ID)%>% inner_join(chemistry %>% select(! col))
AA_COMP_TRANSITIONPOT_GRPS=NINE_LOW_HIGH_DT%>% mutate("10"="-")%>% rbind(TEN_LOW_HIGH_DT)%>% # combine into one data frame
        pivot_longer(cols = `1`:`10`)%>%dplyr::rename(letter=value,position=name)%>% # convert to long format
        inner_join(chemistry)%>% group_by(Dataset, letter, group) %>% dplyr::summarise(n=n())%>% # calculate obs of each amino acid
        rbind(SAMPLED_BACKGROUND_AA_DISTR)%>%
        group_by(Dataset, letter, group)%>% dplyr::summarise(meanN=mean(n), sd=sd(n))%>% # calculate mean and sd of each amino acid per group
        filter(group == "Hydrophobic")%>% # filter to only hydrophobic amino acids
